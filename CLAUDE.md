...
- **AIBlogCard**: Vertical card with Astro `<Image />`, category badge overlay on image. Uses `data-category` for filtering. CSS class `ai-blog-card`
- **AIBlogFilterBar**: Category filter pills + search + month dropdown + tag pills. Client-side JS handles filtering, pagination, and state. Uses `aria-live` regions for screen reader announcements
...
- AIPostLayout uses `<Image>` for hero images; AIBlogCard uses `<Image>` for card thumbnails

### Content Collections
- Schemas defined in `src/content/config.ts` — `ai-blog` and `ai-blog-sources` collections
...
- Export `AIBlogSchema`, `AIBlogSourcesSchema` types
...
- All client-side scripts must re-initialize on `astro:after-swap` event (theme toggle, AI blog filters)
...
- Interactive behavior (theme toggle, AI blog filters) is handled via `<script>` tags in Astro components
...
- AI blog card images use fixed `h-48` container with `object-cover` for consistent sizing
...
### JavaScript
- AI blog filtering is fully client-side — all posts loaded once, no network requests on filter changes
...
- AI Blog uses `pageSize: 999` to generate a single static page (client-side pagination handles the rest)
